...
import {deleteBook, getBookById, getLikesByBookId, getLikesByUserId, likeBook} from "../api/books.js";
...
const detailsTemplate = (book, isOwner, onDelete, likes, showLikeButton, onLike) => html`
    <section id="details-page" class="details">
        <div class="book-information">
            ...
            <p class="img"><img src=${book.imageUrl} alt="bookImg"></p>
            <div class="actions">
                ${bookControlsTemplate(book, isOwner, onDelete)}
                ${likeControlsTemplate(showLikeButton, onLike)}
                <div class="likes">
                    <img class="hearts" src="/images/heart.png" alt="likes">
...
export async function detailsView(ctx) {
    ...
    const [book, likes, isLiked] = await Promise.all([
        ...
        userData ? getLikesByUserId(ctx.params.id, userData.id) : 0
    ]);
    const isOwner = userData?.id == book._ownerId;
    const showLikeButton = userData != null && isOwner == false && isLiked == false;
